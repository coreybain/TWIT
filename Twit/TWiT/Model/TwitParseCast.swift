...
extension TwitEpisodeParse {
    ...
    func parseCast(data:NSArray, complete:([TwitCastDetails]) -> ()) {
        //let castData = data.value as! [String : AnyObject]
        
        var twitParseCastDict: [TwitCastDetails] = []
        
        for castInfo in data {
            
            if let show = castInfo as? NSDictionary {
                ...
                let castID = "\(showValue.value(forKey: "id") as! Int)"
        ...
        if Int(data.count) == twitParseCastDict.count {
